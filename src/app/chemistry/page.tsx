...
export default function AtelierHome() {
  const { hydrated, xp, level, completed, isDone, streak } = useAtelier();
  ...
  return (
    <div className="relative mx-auto max-w-5xl px-4 pb-20 pt-8 sm:px-6">
      {/* floating alchemical particles */}
      {Array.from({ length: 14 }).map((_, i) => (
        <motion.span
          key={i}
          aria-hidden
          className="pointer-events-none absolute h-1.5 w-1.5 rounded-full"
          style={{ left: `${(i * 53) % 100}%`, top: `${(i * 31) % 90}%`, background: "var(--sci-glow)" }}
      ...
      >
        ...
        <p className="mx-auto mt-3 max-w-xl font-hand text-2xl" style={{ color: "var(--sci-accent)" }}>
          Chemistry is the art of transforming matter
        </p>
        <p className="mx-auto mt-4 max-w-2xl italic leading-relaxed opacity-85" style={{ color: "var(--sci-ink)" }}>
          &ldquo;Welcome, apprentice. The furnace is lit and the flasks are
          bubbling. Here we do not memorise — we <em>witness</em>. Come, let us
          discover why matter behaves as it does.&rdquo;
        </p>
      </motion.section>

      {/* Apprentice rank */}
      <section className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <RankCard label="Rank" value={hydrated ? alchemistTitle(level.level) : "Apprentice"} sub={hydrated ? `Level ${level.level}` : ""} icon="🎓" />
        <RankCard label="Elixir (XP)" value={`${xp}`} sub={hydrated ? `${level.into}/${level.span} to next` : ""} icon="✨" />
        <RankCard label="Stations" value={`${doneCount}/${STATIONS.length}`} sub="mastered" icon="⚗️" />
      ...
      <section className="mt-8">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="font-display text-2xl font-extrabold" style={{ color: "var(--sci-ink)" }}>
            Stations of the Atelier
          </h2>
          <Link href="/chemistry/games" className="rounded-full border px-4 py-1.5 text-sm font-bold transition-transform hover:scale-105" style={{ borderColor: "var(--sci-accent)", color: "var(--sci-accent)" }}>
            🎲 Games Room →
          </Link>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          {STATIONS.map((s, i) => {
            ...
            return (
              ...
              >
                <Link href={`/chemistry/lab/${s.id}`} className="block h-full">
                  <div
                    ...
                    style={{
                      ...
                      borderColor: done ? "#10b981" : "var(--sci-border)",
                      color: "var(--sci-ink)",
      ...
      <section className="mt-6">
        <Link href="/chemistry/masterpiece" className="block">
          <div
            className="relative overflow-hidden rounded-2xl border-2 p-6 transition-transform hover:-translate-y-1"
            style={{
              borderColor: "var(--sci-accent)",
              background: "linear-gradient(120deg, color-mix(in srgb, var(--sci-accent) 22%, transparent), var(--sci-panel))",
              color: "var(--sci-ink)",
            }}
          >
